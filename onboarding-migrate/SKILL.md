...
4. Liste `Skills/` (les skills visibles, où le pack est installé) et `.claude/skills/` (normalement que des liens ; un vrai dossier là-dedans est une installation à l'ancienne, à déplacer vers `Skills/` au Step 0). Ne touche à aucun skill pour l'instant. Note la présence de `/save`.
...
6. **Skills** : les skills vivent dans `Skills/` (racine) ou `1 Terrains/<Business>/Skills/` (propres au business : facturation, propositions…), jamais en vrai dossier dans `.claude/skills/`. Déplace ce qui y traîne vers le bon `Skills/`, puis `CLAUDE_PROJECT_DIR="$(pwd)" bash Skills/onboarding/setup/scripts/link-skills.sh`.
