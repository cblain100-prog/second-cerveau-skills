...
8. **Skills du pack.** Le pack livré à la personne, c'est exactement : `onboarding`, `onboarding-migrate`, `app`, `save`, `tri-inbox`, `grill-me`. Ils arrivent dans `.claude/skills/` (dossiers réels). Dans cet ordre : `mkdir -p Skills`, puis pour chaque dossier réel de `.claude/skills/` : `mv .claude/skills/<nom> Skills/<nom>` (y compris `onboarding` lui-même : le fichier en cours de lecture reste lisible), puis `CLAUDE_PROJECT_DIR="$(pwd)" bash Skills/onboarding/setup/scripts/link-skills.sh`. Vérifie : `ls -la .claude/skills/` montre six liens vers `../../Skills/<nom>`. Un skill du pack absent : ne pas le recréer, le dire en Phase 4. Aucun autre skill n'est promis à la personne (pas de `/clean-memory`, pas de `/discover-automations`).
