---
name: save
description: Sauvegarde de fin de session. Lit d'abord la carte de routing du AGENTS.md du vault, puis range AUTOMATIQUEMENT ce qui mérite d'être gardé (décisions, infos, idées, brouillons, ressources) dans les bons dossiers selon cette carte : sans demander. N'écrit JAMAIS dans AGENTS.md ni dans Journal/. Skill général : il s'adapte à n'importe quel vault via son AGENTS.md. Utiliser quand l'utilisateur dit "/save", "save", "sauvegarde", "garde ça", "note ça", "enregistre la session", ou à la fin d'une conversation riche.
---

# Save : Sauvegarde auto fin de session (pilotée par AGENTS.md)

Passe en revue la conversation, identifie ce qui mérite d'être gardé, et range chaque élément dans le bon **dossier** : en suivant la **carte de routing du `AGENTS.md`** de ce vault. À la fin, confirme ce qui a été écrit où.

Ce skill est **général** : il ne connaît pas tes terrains à l'avance. Il les découvre en lisant `AGENTS.md` à chaque exécution. Conséquence : quand ta structure évolue, tu n'as **rien** à changer ici : tu mets juste à jour `AGENTS.md`, et le save suit.

> Ce skill est l'un des deux mécanismes de mémoire du second cerveau. L'AUTRE : la capture des corrections en temps réel : alimente la section « Mémoire » de `AGENTS.md` automatiquement pendant la conversation (Règle #1). Les deux sont **séparés** : /save ne touche jamais `memory.md`.

## Règle dure : /save ne touche QUE les dossiers

- JAMAIS dans `AGENTS.md` (sa section Mémoire est réservée à la Règle #1, mécanisme distinct), ni dans un `memory.md` si le vault en a encore un. /save ne les ouvre même pas.
- JAMAIS dans `Journal/` (pas un récap de session).
- Tout va dans un dossier : terrain, ressource transverse, ou inbox.

Ces trois contraintes priment sur la carte de routing : même si un AGENTS.md disait le contraire, /save n'écrit ni dans `AGENTS.md` ni dans `Journal/`.

## Étape 0 : Lire la carte de routing (AGENTS.md) : OBLIGATOIRE, EN PREMIER

Avant toute écriture, lis le `AGENTS.md` à la racine du vault et repère sa section de **routing** (souvent titrée "Routing : où lire et où écrire" ou équivalent). C'est **LA source de vérité** des destinations dans CE cerveau : elle dit où va chaque type d'information.
...
**Repli** : si `AGENTS.md` est absent, sans section routing, ou trop maigre pour décider → utilise l'**arbre générique par défaut** de l'Étape 2. Ne bloque jamais faute de carte.
...
À la fin, tu listes ce que tu as fait : la personne peut corriger si une destination ne lui plaît pas (ce sera une correction captée par la Règle #1, hors /save : et idéalement répercutée dans AGENTS.md).
...
## Étape 2 : Dispatcher selon AGENTS.md (sinon arbre générique)

Pour chaque élément à garder : applique d'abord la **carte de routing lue à l'Étape 0**. Mappe le type de l'élément à la règle correspondante du AGENTS.md et écris à la destination qu'elle indique (en respectant les cas spéciaux : CRM externe, fichier de facturation, terrain nommé, etc.).
...
Le routing est automatique. Tu ne demandes PAS confirmation. Tu exécutes. Et tu ne touches **ni `AGENTS.md` ni `Journal/`**.
...
1. **Grep** le vault sur ses concepts/noms clés (`grep -ri "[terme]" --include=*.md`, hors `.claude/`, `Skills/` et `2 Archives/`).
...
- 1 Terrains/<terrain>/Docs/<type>/[titre].md : nouveau fichier → relié à [[X]], [[Y]]
...
- **Pas de AGENTS.md / pas de section routing** : utilise l'arbre générique (Étape 2), ne bloque pas.
...
- **AGENTS.md route une info vers `AGENTS.md` ou `Journal/`** : ignore (les contraintes dures priment), range ailleurs (terrain/ressource/inbox).
...
- Contenu vers `AGENTS.md` (réservé à la Règle #1) ou `Journal/` (jamais une cible /save)
