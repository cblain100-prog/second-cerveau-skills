---
name: tri-inbox
description: Vide et trie le dossier 0 Inbox/. Lit d'abord la carte de routing du AGENTS.md du vault, inventorie tout ce qui traîne dans l'inbox (notes .md, images, PDF, exports, dossiers, liens), lit/extrait le contenu de chaque item, puis le range AUTOMATIQUEMENT dans le bon dossier selon cette carte : sans demander. N'écrit JAMAIS dans AGENTS.md ni dans Journal/. Skill général : il s'adapte à n'importe quel vault via son AGENTS.md. Utiliser quand l'utilisateur dit "/tri-inbox", "tri inbox", "trie l'inbox", "traite l'inbox", "vide l'inbox", "range l'inbox", "process inbox".
---

# Tri-inbox : Vide et range le dossier `0 Inbox/` (piloté par AGENTS.md)

Ratisse **le dossier `0 Inbox/`** (pas la conversation), lit chaque item, et le range dans le bon **dossier** : en suivant la **carte de routing du `AGENTS.md`** de ce vault. Sort chaque original de l'inbox une fois traité. À la fin, confirme ce qui est parti où.

Ce skill est **général** : il ne connaît pas tes dossiers à l'avance. Il les découvre en lisant `AGENTS.md` à chaque exécution. Conséquence : quand ta structure évolue, tu n'as **rien** à changer ici : tu mets juste à jour `AGENTS.md`, et le tri suit.

C'est le **jumeau symétrique de `/save`** : même doctrine (lire AGENTS.md, ranger sans demander, relier ce qui a de la valeur), même contraintes dures. Seule différence : la source = le dossier `0 Inbox/`, pas l'historique de conversation.

## Règle dure : tri-inbox ne touche QUE les dossiers

- JAMAIS dans `AGENTS.md` (sa section Mémoire est réservée à la Règle #1, la correction en temps réel), ni dans un `memory.md` si le vault en a encore un.
...
Ces trois contraintes priment sur la carte de routing : même si un AGENTS.md disait le contraire, tri-inbox n'écrit ni dans `AGENTS.md` ni dans `Journal/`.

## Étape 0 : Lire la carte de routing (AGENTS.md) : OBLIGATOIRE, EN PREMIER

Avant de ranger quoi que ce soit, lis le `AGENTS.md` à la racine du vault et repère sa section de **routing** (souvent titrée "Routing : où lire et où écrire" ou équivalent). C'est **LA source de vérité** des destinations dans CE cerveau : elle dit où va chaque type d'information ou d'asset.
...
**Repli** : si `AGENTS.md` est absent, sans section routing, ou trop maigre pour décider → utilise l'**arbre générique par défaut** de l'Étape 3. Ne bloque jamais faute de carte.

## Principe absolu

**Tu classes tout seul. Tu ne demandes JAMAIS "où je le mets ?".** Si tu hésites entre deux destinations, choisis la plus spécifique selon la carte de routing. Si vraiment rien ne matche, l'item **reste dans `0 Inbox/`** (et tu le signales dans le rapport final). À la fin tu listes tout : la personne corrige après si une destination ne lui plaît pas (ce sera une correction à répercuter dans AGENTS.md, hors de ce skill). Git tracke tout si le vault est versionné : rien n'est jamais perdu.
...
## Étape 3 : Dispatcher selon AGENTS.md (sinon arbre générique)

Pour chaque item : applique d'abord la **carte de routing lue à l'Étape 0**. Mappe le type de l'item à la règle correspondante du AGENTS.md et range à la destination qu'elle indique (en respectant les cas spéciaux : CRM externe, dossier client, terrain nommé, etc.).

**Arbre générique par défaut** (uniquement si AGENTS.md ne tranche pas) :

```
1. Info / asset sur un client ou contact nommé ?
   → si ce vault a un CRM externe déclaré dans AGENTS.md, ne crée pas de fichier
...
3. Doctrine / méthode / connaissance réutilisable transverse (sert plusieurs terrains) ?
   → le `Docs/<type>/` du terrain qui s'en servira le plus (il n'y a pas de dossier transverse : si un second terrain en a besoin, un [[wikilink]] suffit) : `1 Terrains/<terrain>/Docs/<type>/[titre-kebab].md`
...
Routing automatique, zéro confirmation. Tu exécutes. Tu ne touches **ni `AGENTS.md` ni `Journal/`**.
...
- [item.md] → 1 Terrains/<terrain>/Docs/<type>/ (déplacé)
...
- **Pas de AGENTS.md / pas de section routing** : utilise l'arbre générique (Étape 3), ne bloque pas.
...
- **Donnée client dynamique** (statut, deal, activité) et AGENTS.md déclare un CRM externe comme source de vérité : ne crée pas de `.md`. Laisse dans l'inbox et signale `"→ à reporter dans le CRM"`.
...
- Écrire dans `AGENTS.md` ou `Journal/`.
- Créer des `.md` de suivi client si AGENTS.md déclare un CRM externe comme source de vérité.
